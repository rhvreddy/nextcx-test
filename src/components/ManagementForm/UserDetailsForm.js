...
const UserDetailsForm = ({
  ...
  const getValidationSchema = (creationType) => {
    return yup.object({
      ...
      businessId: creationType === "superAdmin" ? Yup.string().max(255).required('Business ID is required') : null
      })
  };
  ...
  const formik = useFormik({
    ...
    validationSchema:getValidationSchema(sourcePage),
  ...
  return (
    <form noValidate onSubmit={formik.handleSubmit}>
      <Grid container spacing={3}>
        ...
        {sourcePage && sourcePage === "superAdmin" &&
          <Grid item xs={12}>
            <Stack spacing={1}>
              <InputLabel htmlFor="company-name">Select Business ID*</InputLabel>
              <Select fullWidth displayEmpty
                      value={formik.values.businessId || ''} placeholder="Business ID"
                      name="businessId"
                      disabled={showProgress}
                      ...
                      inputProps={{'aria-label': 'Business ID'}}
                      renderValue={(selected) => {
                        if (!selected) {
                          return <Typography color="text.secondary">Select Business ID</Typography>;
                        }
                        return selected;
                      }}
                      onChange={(e) => {
                        formik.setFieldValue("businessId", e.target.value)
                        businessMenuOptions.filter((item) => {
                          if (item?.businessId === e.target.value) {
                            formik.setFieldValue("companyName", item?.businessName)
                          }
                        })
                      }}
              >
                {businessMenuOptions?.map((item, index) =>
                  <MenuItem value={item?.businessId} key={index}>
                    {item?.businessId}
                  </MenuItem>
                )
                }
              ...
              {formik.touched.businessId && formik.errors.businessId &&
                <FormHelperText error id="helper-text-businessId">{formik.errors.businessId}</FormHelperText>}
            </Stack>
          </Grid>
        }
        <Grid item xs={12}>
        ...
        {sourcePage && sourcePage === "pricing" && <Grid item xs={12}>
          <Stack spacing={1}>
