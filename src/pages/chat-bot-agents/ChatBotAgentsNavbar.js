...
const ChatBotAgentsNavbar = ({open, handleDrawerToggle, items}) => {
  ...
  const handleBotAccessibility = (botProperties)=>{
    if(botProperties?.status){
      if(botProperties?.status?.toLowerCase() === "error" || botProperties?.status?.toLowerCase() === "in-progress"){
        return true;
      }
      else{
        return false;
        ...
        (botList?.length > 0 ? <Stack justifyContent="center" alignItems="start" width="100%"> <List
          ...
          }}>
          {
            botList?.map((bot, index) =>
              <ListItemButton disabled={handleBotAccessibility(bot)} key={index} onClick={() => handleClickBotId(bot?.id)} sx={{
                ...
                <Avatar alt="Avatar" src={bot?.profileURL || "https://d3dqyamsdzq0rr.cloudfront.net/images/bot_icon.png"}
          ...
          <Box sx={{display: "flex", justifyContent: "center", alignItems: "center", height: "70vh"}}><Typography
            color="secondary" fontWeight="600" fontSize="16px">{drawerOpen ? "No bots found" : ""}</Typography></Box>)
      }
    </Grid>
  )
};
...
export default ChatBotAgentsNavbar;
